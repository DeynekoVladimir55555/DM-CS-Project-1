class NaturalNumber:
    ...
    def nzer_n_b(self):  # Является ли число нулём?
        if self.digits[0] != 0:
            return "да"
        return "нет"
    def add_1n_n(number):  # Прибавление к натуральному числу единицы
...
def COM_NN_D(number1, number2):  # Сравнение двух натуральных чисел
...
def ADD_NN_N(number1, number2): # Сумма двух натуральных чисел
    tmp = 0
    result = []
